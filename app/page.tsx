      ...
      <div className="flex items-center justify-between px-3 md:px-4 py-2 bg-gray-900 border-b border-gray-800 shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <a
              href={`/flashcards?profile=${encodeURIComponent(game.profile)}&language=${encodeURIComponent(game.languageId)}&module=${encodeURIComponent(game.module)}`}
              className="text-xs text-violet-400 hover:text-violet-300 transition-colors flex items-center gap-1"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <rect x="2" y="6" width="20" height="12" rx="2" />
                <path strokeLinecap="round" d="M6 2v4M18 2v4" />
              </svg>
              Cards
            </a>
          ...
          <div className="relative md:hidden">
            ...
            {mobileMenuOpen && (
              <div className="absolute right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl py-1 z-50 min-w-[200px]">
                ...
                <a
                  href={`/flashcards?profile=${encodeURIComponent(game.profile)}&language=${encodeURIComponent(game.languageId)}&module=${encodeURIComponent(game.module)}`}
                  className="flex items-center gap-2 px-4 py-2 text-sm text-violet-400 hover:bg-gray-700"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <rect x="2" y="6" width="20" height="12" rx="2" />
