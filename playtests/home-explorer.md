...
## Addendum: Second Explorer Playtest (2026-02-11)

A second playtest was conducted as Sofia with ~30 turns across two sessions. The first session crashed on turn 23 and required a full restart. The second session covered 24 additional turns. Key new findings below.

### NEW CRITICAL: Crash on compound room-change + NPC reaction

- **Severity**: CRITICAL
- **Trigger**: "Ir a la sala y encender la televisión" (go to living room AND turn on TV in one command)
- **Error**: `Cannot read properties of undefined (reading 'charAt')` at `ChatPanel.tsx:106` in `TurnFeedback` component
- **Root cause**: The code calls `result.npcResponse.npcName.charAt(0).toUpperCase()` but when a compound action triggers both a room change and an NPC reaction in the same turn, the `npcResponse` object lacks the `npcName` field.
- **Repro**: Start fresh, get out of bed, go to living room, type "Ir a la sala y encender la televisión"
- **Impact**: Unrecoverable crash. Error overlay covers the entire page, input field is inaccessible, session must be restarted from scratch.
- **Note**: Turning on the TV as a simple action while already in the living room works fine. The crash is specific to compound actions that combine room movement with an action that triggers NPC commentary.

### NEW HIGH: Kitchen has no sink

- **Severity**: HIGH (immersion-breaking)
- **Details**: "Lavar mis manos en el fregadero" in the kitchen returns "There is no sink in the kitchen. The sink is in the bathroom."
- **Impact**: A kitchen without a sink is unrealistic and breaks immersion.

### NEW MEDIUM: Wardrobe contents not interactive

- **Details**: Opening the wardrobe describes clothes (shirts, pants, sweater) but trying to take any item says nothing is inside. The narrative creates expectations the object system cannot fulfill.

### NEW MEDIUM: Goal says "baño caliente" but bathroom has shower only

- **Details**: Goal "Tomar un baño caliente" (take a hot bath) contradicts the bathroom only having a shower. "Abrir el grifo de la bañera" is rejected with "there's only a shower."
- **Fix**: Change goal text to "Tomar una ducha caliente" or add a bathtub.

### NEW MEDIUM: Fridge description inconsistent with contents

- **Details**: Initial fridge description mentioned cheese and leftovers, but actual objects are milk, eggs, butter, and jam. Players who read carefully will notice the mismatch.

### Additional Positive Observations

- **Alarm clock blocks sleeping**: Trying to sleep with the alarm ringing gets a logical refusal ("turn it off first"). Feels like a real game mechanic.
- **Window state gating**: Must open the window before looking outside. Natural and immersive.
- **Bed state toggling**: Can get back in bed after getting up. The scene image updates correctly.
- **Coffee maker chain**: Make coffee then drink it works as a two-step process.
- **Inanimate rejection is clean**: "Hablar con la estufa" gets "it's just a kitchen appliance, not a person or pet" - clear, not condescending.
- **Street boundary handling**: "Hablar con alguien en la calle" correctly says nobody is around and suggests going to specific locations.
- **Cat interactions are rich**: Talk, pet, pick up Luna all work with charming, distinct responses. Carlos even comments when you hold Luna.
